...
router.get('/readiness/:opportunityId', requireAuth, async (req, res) => {
  try {
    const { data, error } = await supabase.from('readiness_assessments').select('*').eq('opportunity_id', req.params.opportunityId).order('assessed_at', { ascending: false }).limit(1).single();
...
router.get('/intelligence', requireAuth, async (req, res) => {
  try {
    ...
    const { data: events } = await supabase.from('capital_intelligence_events').select('event_type,metadata,recorded_at').order('recorded_at',{ascending:false}).limit(20);
